...
import type { Address, PolicyId, TokenId, TransactionId, Utxo } from '@/@types'
    ...
    for (const output of txUtxos.outputs) {
      ...
      if (toAddress !== fromAddress) {
        ...
        output.amount.forEach(({ unit: tokenId, quantity }) => {
          ...
          if (tokenAmount && policyIds.some((policyId) => tokenId.indexOf(policyId) === 0) && !!input.amount.find((inp) => inp.unit === tokenId)) {
            tokens.push({
              tokenId,
              tokenAmount,
            })
